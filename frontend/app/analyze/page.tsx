...
function SectionHeader({ icon, title, badge }: { icon: React.ReactNode; title: string; badge?: string }) {
  return (
    <div className="mb-5 flex items-center gap-2.5">
      ...
      {badge && (
        <span className="rounded-full bg-zinc-800 px-2 py-0.5 text-xs text-zinc-500">{badge}</span>
      )}
    </div>
  );
}
...
function KeywordBadge({ keyword, variant = "default" }: { keyword: string; variant?: "default" | "warning" | "danger" }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-md border px-2 py-1 text-xs font-medium ${styles[variant]}`}>
      {keyword}
    </span>
  );
}
    ...
    <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {items.map((item, i) => {
          ...
          const rateColor =
            i === 3
              ? item.value / (items[2].value || 1) > 0.2
                ? "text-red-400"
                : "text-emerald-400"
            ...
            <div key={item.label} className="flex items-center gap-2">
              {i > 0 && (
                <div className="flex flex-col items-center">
                  ...
                  {rate && (
                    <span className={`text-xs tabular-nums ${rateColor}`}>{rate}%</span>
                  )}
                </div>
              )}
  ...
  return (
    <InsightCard title="Bütçe Verimlilik Skoru">
      <div className="flex items-end gap-4">
        <div>
          <p className={`text-4xl font-bold tabular-nums ${textColor}`}>{score.toFixed(0)}</p>
        ...
        <div className="mx-auto flex max-w-4xl items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-sm font-semibold text-zinc-200">doThis</span>
            </div>
            <span className="text-zinc-700">·</span>
            {result.used_fixture && (
              <span className="rounded-full border border-amber-500/20 bg-amber-500/8 px-3 py-1 text-xs font-medium text-amber-400">
                Demo modu
              </span>
            )}
            {adTypeLabel && (
      ...
      <main className="mx-auto max-w-4xl px-6 py-10">
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-col gap-3">
            {/* Score + suggested title row */}
            <div className="grid gap-3 sm:grid-cols-2">
              <InsightCard title="Mevcut Skor">
                <div className="flex items-end gap-3">
                  <p className={`text-5xl font-bold tabular-nums ${result.geo_score >= 70 ? "text-emerald-400" : result.geo_score >= 40 ? "text-amber-400" : "text-red-400"}`}>
                    {result.geo_score}
                  </p>
                  <div className="pb-1">
                    <p className="text-xs text-zinc-500">/100 · GEO skoru</p>
                    <p className="text-xs text-zinc-600">
                      {result.geo_score >= 70 ? "İyi görünürlük" : result.geo_score >= 40 ? "Orta görünürlük" : "Düşük görünürlük"}
                    </p>
                  </div>
                </div>
                <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-zinc-800">
                  <div
                    className={`h-full rounded-full transition-all ${result.geo_score >= 70 ? "bg-emerald-500" : result.geo_score >= 40 ? "bg-amber-500" : "bg-red-500"}`}
                    style={{ width: `${result.geo_score}%` }}
                  />
                </div>
              </InsightCard>

              {result.geo_suggested_title && (
                <InsightCard title="Önerilen Başlık">
                  <p className="text-sm font-medium leading-relaxed text-zinc-200">{result.geo_suggested_title}</p>
                  {result.geo_suggested_description && (
                    <p className="mt-2 text-xs leading-relaxed text-zinc-500">{result.geo_suggested_description}</p>
                  )}
                </InsightCard>
              )}
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {result.geo_missing_keywords?.length > 0 && (
                <InsightCard title="Eksik Anahtar Kelimeler">
                  <div className="flex flex-wrap gap-1.5">
                    {result.geo_missing_keywords.map((kw) => (
                      <KeywordBadge key={kw} keyword={kw} variant="warning" />
                    ))}
                  </div>
                </InsightCard>
              )}
              {result.geo_competitor_keywords?.length > 0 && (
                <InsightCard title="Rakip Anahtar Kelimeleri">
                  <div className="flex flex-wrap gap-1.5">
                    {result.geo_competitor_keywords.map((kw) => (
                      <KeywordBadge key={kw} keyword={kw} variant="default" />
                    ))}
                  </div>
                </InsightCard>
              )}
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                {result.budget_efficiency_score !== null && result.budget_efficiency_score !== undefined && (
                  <BudgetEfficiencyCard
                    score={result.budget_efficiency_score}
                    funnel_drop_points={result.funnel_drop_points ?? []}
                  />
                )}
              </div>

              {/* Keyword ROI Table */}
              {result.keyword_roi_map?.length > 0 && (
                <KeywordRoiTable data={result.keyword_roi_map} />
              )}
